...
def main() -> int:
    parser = argparse.ArgumentParser(description="Feature 20 cost attribution gate")
    parser.add_argument("--mode", choices=["ci", "preflight"], required=True)
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--metering", required=True)
    ...
    report = evaluate_cost_attribution_gate(
        run_id=args.run_id,
        mode=args.mode,
        metering=_read_json(Path(args.metering)),
        aggregation=_read_json(Path(args.aggregation)),
        caps=_read_json(Path(args.caps)),
    )
    errors = validate_cost_attribution_report_dict(report)
    if errors:
        print("cost attribution contract invalid:")
        for error in errors:
            print(f"- {error}")
        return 2
    ...
    contract_path = out_path.parent / "cost_contract.json"
    contract_path.write_text(
        json.dumps(build_cost_attribution_contract(), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    events_path = out_path.parent / "cost_events.jsonl"
    event_rows = build_cost_event_rows(report=report)
    ...
    updated = update_cost_attribution_history(existing=existing, report=report)
