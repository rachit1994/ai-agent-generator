...
def main() -> int:
    parser = argparse.ArgumentParser(description="Feature 11 plugin registry governance gate")
    ...
    report = evaluate_plugin_registry_gate(
        run_id=args.run_id,
        mode=args.mode,
        registry_state=_read_json(Path(args.registry)),
        compatibility_matrix=_read_json(Path(args.compatibility)),
        governance=_read_json(Path(args.governance)),
    )
    errors = validate_plugin_registry_report_dict(report)
    if errors:
        print("plugin registry contract invalid:")
        for error in errors:
            print(f"- {error}")
        return 2
    ...
    contract_path = out_path.parent / "registry_contract.json"
    contract_path.write_text(
        json.dumps(build_plugin_registry_contract(), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    events_path = out_path.parent / "registry_events.jsonl"
    event_rows = build_registry_event_rows(report=report)
    ...
    updated = update_plugin_registry_history(existing=existing, report=report)
