...
def main() -> int:
    parser = argparse.ArgumentParser(description="Feature 18 cross-tenant isolation gate")
    ...
    report = evaluate_cross_tenant_isolation_gate(
        run_id=args.run_id,
        mode=args.mode,
        execution_context=_read_json(Path(args.execution)),
        isolation_state=_read_json(Path(args.isolation)),
        audit_signals=_read_json(Path(args.audit)),
    )
    errors = validate_cross_tenant_isolation_report_dict(report)
    if errors:
        print("cross-tenant isolation contract invalid:")
        for error in errors:
            print(f"- {error}")
        return 2
    ...
    contract_path = out_path.parent / "isolation_contract.json"
    contract_path.write_text(
        json.dumps(build_cross_tenant_contract(), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    events_path = out_path.parent / "isolation_events.jsonl"
    events = build_isolation_event_rows(report=report)
    ...
    updated = update_cross_tenant_isolation_history(existing=existing, report=report)
