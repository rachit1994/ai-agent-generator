...
def main() -> int:
    parser = argparse.ArgumentParser(description="Feature 03 control/data plane split gate")
    ...
    report = evaluate_plane_split_gate(
        run_id=args.run_id,
        mode=args.mode,
        control=_read_json(Path(args.control)),
        data=_read_json(Path(args.data)),
    )
    errors = validate_plane_split_report_dict(report)
    if errors:
        print("plane split contract invalid:")
        for error in errors:
            print(f"- {error}")
        return 2
    ...
    boundary_path = out_path.parent / "boundary_contract.json"
    boundary_path.write_text(
        json.dumps(build_plane_boundary_contract(), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    events_path = out_path.parent / "plane_events.jsonl"
    event_rows = build_plane_event_rows(report=report)
    ...
    updated = update_plane_split_history(existing=existing, report=report)
