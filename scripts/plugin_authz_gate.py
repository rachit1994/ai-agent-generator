...
def main() -> int:
    parser = argparse.ArgumentParser(description="Feature 17 plugin authz gate")
    ...
    report = evaluate_plugin_authz_gate(
        ...
        policy_state=_read_json(Path(args.policy)),
        audit_logs=_read_json(Path(args.audit)),
    )
    errors = validate_plugin_authz_report_dict(report)
    if errors:
        print("plugin authz contract invalid:")
        for error in errors:
            print(f"- {error}")
        return 2
    ...
    contract_path = out_path.parent / "authz_contract.json"
    contract_path.write_text(
        json.dumps(build_plugin_authz_contract(), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    events_path = out_path.parent / "authz_events.jsonl"
    event_rows = build_authz_event_rows(report=report)
    ...
    updated = update_plugin_authz_history(existing=existing, report=report)
