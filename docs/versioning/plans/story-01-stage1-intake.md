...
---
*Generated by `docs/versioning/_generate_plans.py` — safe to edit this file in place after generation. A **full** generator run skips overwriting this slug while it is listed in **`CURATED_PLAN_SLUGS`** in `_generate_plans.py`. To refresh **`docs/versioning/INDEX.md`** only (without touching plan bodies), run: `python3 docs/versioning/_generate_plans.py --index-only`.*
