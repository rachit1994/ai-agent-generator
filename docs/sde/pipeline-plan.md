...
This document is the **single planning-agent output**: it defines the order of work and what may run in parallel to ship the **execution slice** (artifacts + gates in **`src/sde_gates/`** and **`docs/sde/implementation-contract.md`**). For the **full program story** (parallel agents, capability metrics), see [`docs/onboarding/action-plan.md`](../onboarding/action-plan.md).

## Sequential spine (blocking order)

These steps must complete in order; later steps assume earlier contracts exist.

1. **Contract lock** — Freeze **`docs/sde/implementation-contract.md`** and gate behavior in **`src/sde_gates/`** (artifacts, CTO hard-stops, balanced gates).
