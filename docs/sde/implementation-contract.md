...
- Required artifacts:
  ...
  - **`sde run --repeat N`**: **N** independent runs (each its own `run-manifest.json` and `outputs/runs/<run-id>/`); JSON envelope when **N > 1** (see `docs/coding-agent/execution.md` RepeatProfile)
