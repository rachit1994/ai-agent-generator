...
**How to read this (humans and LLMs):** it is **long on purpose** — treat it as a **reference library**, not a tutorial. Read **§1–2** first for intent, then jump by section when you need depth. For **what this repository’s Python CLI actually does**, read **`docs/UNDERSTANDING-THE-CODE.md`** (gates and behavior live in **`src/guardrails_and_safety/`**). External papers and library notes: **`docs/RESEARCH.md`**. When you summarize, separate **(a) behaviors we can prove from files and gates** from **(b) named infrastructure** (databases, vendors) that may still be **choices** per environment.
...
- **`docs/UNDERSTANDING-THE-CODE.md`** — SDE baseline (`what.md` content), implementation contract, project driver, Stage 1 runbook (single file).
- **`src/guardrails_and_safety/`** — CTO gates and hard-stops (**HS01+**).
- **`src/workflow_pipelines/production_pipeline_task_to_promote/runner/`** — run lifecycle and harness layers (`*_layer.py`).

Phase exits in §17 should cite **concrete evidence paths** (artifacts, gate IDs, replay manifests) produced by the **Python** paths above. The repository keeps **four** Markdown files under `docs/`; this file is the **architecture source of truth**.

**Implementation spine (this repo):** Treat **V1 through V7** themes as **one combined ladder** toward full-stack, production-gated delivery—not seven isolated doc drops. **Execution safety and HS01–HS06 dominate** later behavior. **End-to-end product flow** is described in **§2** and **§10** of this document (stages, pipelines, gates).
...
**Implementation % map (this repo vs this blueprint):** [docs/architecture/master-architecture-feature-completion.md](architecture/master-architecture-feature-completion.md).
...
repo/
  docs/
    AI-Professional-Evolution-Master-Architecture.md   # architecture source of truth
    UNDERSTANDING-THE-CODE.md                          # CLI + src map + contracts
    RESEARCH.md                                        # papers / libraries
    architecture/
      master-architecture-feature-completion.md        # % done vs this blueprint
  contracts/
    event/
    policy/
    lifecycle/
