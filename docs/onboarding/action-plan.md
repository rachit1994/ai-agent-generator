...
**Shipped slice (local CLI):** session-level **`sde project run`** + **`project_plan.json` / `progress.json`** driver, bounded **ContextPack**, orchestrator **per-step shell verification**, **`sde project validate`** (CI preflight, optional **`--require-plan-lock`** / **`--require-non-stub-reviewer`** for Stage 1), **`sde project plan-lock`** / **`project_plan_lock.json`**, **`sde project status`** (read-only JSON, including **`status_at_a_glance`** for quick health), optional **`sde continuous --project-plan`** (same **`--enforce-plan-lock`** / strict reviewer pairing as **`project run`**), optional **`--parallel-worktrees`** when git + disjoint scopes apply — see [`../sde/project-driver.md`](../sde/project-driver.md) (Stage 1 plan lock section) and [`../runbooks/stage1-intake-failures.md`](../runbooks/stage1-intake-failures.md). This is still mostly a **sequential** meta-orchestrator on one checkout; the full parallel-lane + heartbeat story above remains roadmap beyond the shipped lease + worktree slices.
