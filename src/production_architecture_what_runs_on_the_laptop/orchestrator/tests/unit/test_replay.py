...
from workflow_pipelines.production_pipeline_task_to_promote.benchmark.task_loop import synthetic_finalize_failure_event
from workflow_pipelines.production_pipeline_task_to_promote.config import DEFAULT_CONFIG
from workflow_pipelines.production_pipeline_task_to_promote.replay import replay_run
...
def test_write_trajectory_html_creates_file(tmp_path: Path, monkeypatch) -> None:
    ...
    (run_dir / "traces.jsonl").write_text(json.dumps(ev) + "\n", encoding="utf-8")
    from workflow_pipelines.production_pipeline_task_to_promote.replay import write_trajectory_html
...
def test_replay_rerun_invokes_execute(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("workflow_pipelines.production_pipeline_task_to_promote.runner.execute_single_task", _fake_execute)
