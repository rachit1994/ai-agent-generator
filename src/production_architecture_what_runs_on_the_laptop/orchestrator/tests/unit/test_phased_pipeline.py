...
from workflow_pipelines.execution_modes.modes.phased_pipeline import run_phased_pipeline
from workflow_pipelines.execution_modes.modes.phased_pipeline.decompose import flatten_plan, fallback_single_todo_plan
...
def test_run_phased_pipeline_runs_guarded_per_todo(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "workflow_pipelines.execution_modes.modes.phased_pipeline.pipeline.run_decompose_phase",
        _fake_decompose,
    )
    monkeypatch.setattr(
        "workflow_pipelines.execution_modes.modes.phased_pipeline.pipeline.run_guarded_pipeline",
        _fake_guarded,
    )
