...
from workflow_pipelines.production_pipeline_task_to_promote.config import config_snapshot
from workflow_pipelines.production_pipeline_task_to_promote.run_logging import (
...
from storage.storage import append_jsonl, ensure_dir, read_json, read_jsonl, write_json
from common.utils import create_run_id, outputs_base
from workflow_pipelines.production_pipeline_task_to_promote.runner.traces_jsonl_event_contract import (
...
def _write_benchmark_checkpoint(ck_path: Path, body: dict[str, Any]) -> None:
    ck_errs = validate_benchmark_checkpoint_dict(body)
    if ck_errs:
...
def _write_benchmark_summary(out_dir: Path, body: dict[str, Any]) -> None:
    ag_errs = validate_benchmark_aggregate_summary_dict(body)
    if ag_errs:
...
def _bench_ctx_resume(resume_run_id: str, suite_path: str | None, mode: str) -> _BenchCtx:
    ...
    bm_errs = validate_benchmark_manifest_dict(manifest)
    if bm_errs:
        raise ValueError(f"benchmark_manifest_contract:{','.join(bm_errs)}")
...
def _bench_ctx_fresh(
    ...
    bm_errs = validate_benchmark_manifest_dict(bench_manifest)
    if bm_errs:
        raise ValueError(f"benchmark_manifest_contract:{','.join(bm_errs)}")
    write_json(out / _BENCHMARK_MANIFEST, bench_manifest)
    ck_path = out / _BENCHMARK_CHECKPOINT
    completed_ids: list[str] = []
    ...
    *,
    ...
    def on_task_events(tid: str, baseline_chunk: list[dict], guarded_chunk: list[dict]) -> None:
        for ev in baseline_chunk:
            te_errs = validate_traces_jsonl_event_dict(ev)
            if te_errs:
                raise ValueError(f"traces_jsonl_event_contract:{','.join(te_errs)}")
            append_jsonl(out / _TRACES_JSONL, ev)
        for ev in guarded_chunk:
            te_errs_g = validate_traces_jsonl_event_dict(ev)
            if te_errs_g:
                raise ValueError(f"traces_jsonl_event_contract:{','.join(te_errs_g)}")
            append_jsonl(out / _TRACES_JSONL, ev)
