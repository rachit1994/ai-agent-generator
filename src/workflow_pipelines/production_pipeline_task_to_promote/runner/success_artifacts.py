...
from artifacts.artifacts import extract_python_code
...
from workflow_pipelines.production_pipeline_task_to_promote.report import generate_report
from storage.storage import append_jsonl, ensure_dir, write_json
...
def write_success_artifact_layer(
    *,
    ...
    run_end_errs = validate_orchestration_run_end_dict(run_end_body)
    if run_end_errs:
        raise ValueError(f"orchestration_run_end_contract:{','.join(run_end_errs)}")
    append_jsonl(orchestration, run_end_body)
