...
def test_run_directory_surfaces_missing_strategy_overlay_evidence_ref_file(
    tmp_path: Path, monkeypatch
) -> None:
    out = _baseline_dir(tmp_path)
    (out / "strategy" / "overlay.json").write_text(
        json.dumps(
            {
                "schema": "sde.strategy_overlay_runtime.v1",
                "schema_version": "1.0",
                "run_id": "rid-1",
                "mode": "baseline",
                "strategy_name": "hold",
    ...
    assert "strategy_overlay_runtime_contract:strategy_overlay_runtime_evidence_ref:proposal_ref_missing" in result["errors"]
