...
def test_run_directory_invalid_storage_artifact_surfaces_storage_contract_prefix(
    tmp_path: Path, monkeypatch
) -> None:
    out = _baseline_dir(tmp_path)
    (out / "storage" / "storage_architecture.json").write_text(json.dumps({"schema": "bad"}), encoding="utf-8")
    monkeypatch.setattr(
    ...
    assert any(err.startswith("storage_contract:") for err in result["errors"])
