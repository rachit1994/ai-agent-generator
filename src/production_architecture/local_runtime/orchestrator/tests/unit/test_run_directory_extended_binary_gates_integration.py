...
def test_run_directory_reports_missing_extended_binary_gates_evidence_refs(
    ...
    (out / "learning" / "extended_binary_gates.json").write_text(json.dumps(payload), encoding="utf-8")
    monkeypatch.setattr(
    ...
    assert (
        "extended_binary_gates_contract:extended_binary_gates_evidence_ref:skill_nodes_ref_missing"
        in result["errors"]
    )
