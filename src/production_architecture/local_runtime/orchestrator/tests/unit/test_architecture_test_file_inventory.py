...
def _part_a_py_md_paths(root: Path) -> list[Path]:
    """Same set as ``find src ( -name '*.py' -o -name '*.md' ) ! -path '*/__pycache__/*'``."""
    out: list[Path] = []
...
def test_layout_inventory_part_a_path_count_matches_disk() -> None:
    """Snapshot count for ``.py`` / ``.md`` files under ``src/`` (see inventory Part A intro)."""
    root = _repo_root()
...
def test_architecture_doc_test_py_inventory_matches_disk() -> None:
    """Snapshot: **71** `test_*.py` files (**70** under orchestrator `tests/`, **1** under repo-root `tests/`)."""
    root = _repo_root()
    orch_tests = root / "src" / "production_architecture" / "local_runtime" / "orchestrator" / "tests"
    ...
    assert orch_count + top_count == 71
