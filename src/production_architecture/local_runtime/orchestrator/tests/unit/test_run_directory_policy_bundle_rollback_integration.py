...
def test_run_directory_surfaces_atomic_source_vs_derived_mismatch(
    ...
    result = validate_execution_run_directory(out, mode="baseline")
    assert "rollback_rules_policy_bundle_source_status_mismatch:rolled_back_atomic" in result["errors"]


def test_run_directory_rejects_forged_none_status_checks(tmp_path: Path, monkeypatch) -> None:
    out = _baseline_dir(tmp_path)
    (out / "program" / "policy_bundle_rollback.json").write_text(
        json.dumps({"schema_version": "1.0", "status": "none"}),
        encoding="utf-8",
    ...
    result = validate_execution_run_directory(out, mode="baseline")
    assert "rollback_rules_policy_bundle_source_derivation_mismatch" in result["errors"]


def test_run_directory_rejects_policy_bundle_coherence_when_source_unreadable(
    tmp_path: Path, monkeypatch
) -> None:
    out = _baseline_dir(tmp_path)
    (out / "program" / "policy_bundle_rollback.json").write_text("{", encoding="utf-8")
    ...
    result = validate_execution_run_directory(out, mode="baseline")
    assert "policy_bundle_rollback_invalid_json" in result["errors"]
    assert "rollback_rules_policy_bundle_source_unreadable" in result["errors"]


def test_run_directory_rejects_policy_bundle_coherence_when_derived_run_id_blank(
    tmp_path: Path, monkeypatch
) -> None:
    out = _baseline_dir(tmp_path)
    (out / "program" / "policy_bundle_rollback.json").write_text(
    ...
    result = validate_execution_run_directory(out, mode="baseline")
    assert "rollback_rules_policy_bundle_contract:rollback_rules_policy_bundle_run_id" in result["errors"]
    assert "rollback_rules_policy_bundle_source_run_id_invalid" in result["errors"]
