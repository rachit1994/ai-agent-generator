...
def test_run_directory_surfaces_invalid_orchestrator_component_contract(
    tmp_path: Path, monkeypatch
) -> None:
    out = _baseline_dir(tmp_path)
    (out / "orchestrator").mkdir(parents=True, exist_ok=True)
    (out / "orchestrator" / "component_runtime.json").write_text(
    ...
    assert "orchestrator_component_contract:orchestrator_component_schema" in result["errors"]
