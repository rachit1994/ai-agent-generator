...
def test_run_directory_surfaces_memory_system_contract_errors(tmp_path: Path, monkeypatch) -> None:
    out = _baseline_dir(tmp_path)
    (out / "memory").mkdir(parents=True)
    (out / "memory" / "memory_system.json").write_text(json.dumps({"schema": "bad"}), encoding="utf-8")
    monkeypatch.setattr(
    ...
    assert "memory_system_contract:memory_system_schema" in result["errors"]


def test_run_directory_requires_memory_system_path(tmp_path: Path, monkeypatch) -> None:
    out = _baseline_dir(tmp_path)
    monkeypatch.setattr(
        "guardrails_and_safety.review_gating_evaluator_authority.review_gating.run_directory.all_required_execution_paths",
        lambda mode, output_dir: ["memory/memory_system.json"],
    )
    monkeypatch.setattr(
        "guardrails_and_safety.review_gating_evaluator_authority.review_gating.run_directory.evaluate_hard_stops",
        lambda output_dir, events, token_ctx, run_status, mode: [],
