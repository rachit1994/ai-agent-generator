...
def test_run_directory_reports_missing_role_agents_evidence_refs(
    ...
    payload = {
        ...
        "run_id": "rid-1",
        "status": "balanced",
        "role_scores": {"planner": 1.0, "implementor": 1.0, "verifier": 1.0},
        "evidence": {
    ...
    (out / "capability" / "role_agents.json").write_text(json.dumps(payload), encoding="utf-8")
    monkeypatch.setattr(
    ...
    assert "role_agents_contract:role_agents_evidence_ref:skill_nodes_ref_missing" in result["errors"]
