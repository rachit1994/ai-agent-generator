...
def _baseline_dir(tmp_path: Path) -> Path:
    ...
    (out / "run-manifest.json").write_text(json.dumps({"schema": "bad"}), encoding="utf-8")
    return out


def test_run_directory_surfaces_invalid_run_manifest_contract(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "run_manifest_contract:run_manifest_schema" in result["errors"]


def test_run_directory_surfaces_invalid_run_manifest_runtime_contract(
    ...
    (out / "program" / "run_manifest_runtime.json").write_text(json.dumps({"schema": "bad"}), encoding="utf-8")
    monkeypatch.setattr(
        "guardrails_and_safety.review_gating_evaluator_authority.review_gating.run_directory.all_required_execution_paths",
        lambda mode, output_dir: [],
    )
    ...
    result = validate_execution_run_directory(out, mode="baseline")
    assert "run_manifest_runtime_contract:run_manifest_runtime_schema" in result["errors"]
