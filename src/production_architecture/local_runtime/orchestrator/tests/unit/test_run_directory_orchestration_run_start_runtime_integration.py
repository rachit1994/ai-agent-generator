...
def test_run_directory_surfaces_invalid_orchestration_run_start_runtime_contract(
    tmp_path: Path, monkeypatch
) -> None:
    out = _baseline_dir(tmp_path)
    (out / "orchestration").mkdir(parents=True, exist_ok=True)
    (out / "orchestration" / "run_start_runtime.json").write_text(
    ...
    assert "orchestration_run_start_runtime_contract:orchestration_run_start_runtime_schema" in result["errors"]
