...
def test_regression_testing_surface_written_and_valid(tmp_path: Path, monkeypatch) -> None:
    ...
    write_json(run_dir / "learning" / "online_evaluation_shadow_canary.json", {"status": "promote"})
...
def test_regression_testing_surface_degraded_when_eval_payloads_lack_status(
    tmp_path: Path, monkeypatch
) -> None:
    run_id = "run-regression-testing-surface-degraded"
    run_dir = tmp_path / "runs" / run_id
    ensure_dir(run_dir)
    for _dimension, rel in REGRESSION_DIMENSION_ANCHORS:
        anchor_path = tmp_path / rel
    ...
    write_json(run_dir / "learning" / "online_evaluation_shadow_canary.json", {"result": "promote"})
    payload = write_regression_testing_surface_artifact(output_dir=run_dir, run_id=run_id)
