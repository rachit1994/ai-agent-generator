...
def _baseline_dir(tmp_path: Path) -> Path:
    out = tmp_path / "run"
    (out / "event_store").mkdir(parents=True)
...
def test_run_directory_surfaces_invalid_event_store_semantics_contract(
    tmp_path: Path, monkeypatch
) -> None:
    out = _baseline_dir(tmp_path)
    (out / "event_store" / "semantics.json").write_text(json.dumps({"schema": "bad"}), encoding="utf-8")
    monkeypatch.setattr(
    ...
    assert "event_store_semantics_contract:event_store_semantics_schema" in result["errors"]


def test_run_directory_surfaces_missing_event_store_semantics_evidence_refs(
    tmp_path: Path, monkeypatch
) -> None:
    out = _baseline_dir(tmp_path)
    (out / "event_store" / "semantics.json").write_text(
        json.dumps(
            {
                "schema": "sde.event_store_semantics.v1",
                "schema_version": "1.0",
                "run_id": "rid-1",
                "status": "broken",
                "metrics": {
                    ...
                    "trace_count": 0,
                    "coverage": 0.0,
                    "chain_root_matches_source": False,
                },
                "evidence": {
    ...
    result = validate_execution_run_directory(out, mode="baseline")
    assert (
        "event_store_semantics_contract:event_store_semantics_evidence_ref:replay_manifest_ref_missing"
        in result["errors"]
    )
