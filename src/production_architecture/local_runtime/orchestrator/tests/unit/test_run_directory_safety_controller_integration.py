...
def test_run_directory_surfaces_invalid_safety_controller_contract(tmp_path: Path, monkeypatch) -> None:
    out = _baseline_dir(tmp_path)
    (out / "safety").mkdir(parents=True, exist_ok=True)
    (out / "safety" / "controller_runtime.json").write_text(
    ...
    assert "safety_controller_contract:safety_controller_schema" in result["errors"]
